<template>
  <div>
    ...
    <div class="column is-narrow">
      <label class="label">Categories</label> Category: {{ item.category.name
      }}<br />
      <v-select
        label="slug"
    ...
    <div class="column is-narrow">
      <label class="label">User</label> Username: {{ item.user.username }}<br />
      <v-select
        v-model="item.user"
        :debounce="250"
    ...
    <div class="field">
      <div class="control">
        <label class="checkbox">
          <input v-model="item.isActive" type="checkbox" /> Active
        </label>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getOptions(search, loading) {
        loading(true);
        axios.get(`users?email=${search}`).then(resp => {
